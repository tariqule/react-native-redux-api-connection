...
const ENDPOINT = `http://67.205.148.211`;
...
export const DELETE_REQUEST = `POSTS_REQUEST`;
export const DELETE_REQUEST_SUCCESS = `POSTS_REQUEST_SUCCESS`;
export const DELETE_REQUEST_FAILURE = `POSTS_REQUEST_FAILURE`;



export const deleteRequestBegin = () => ({
    type: DELETE_REQUEST
...
export const deleteCustomer = (data, id) => {
    return dispatch => {
        dispatch(deleteRequestBegin());
        axios
            .delete(`${ENDPOINT}/login/${id}/delete`, data)  // axios.post takes the URL and and the data object
            .then(res => {         //the res is the result that we get after sending the post request
                setTimeout(() => {
                    dispatch(deleteRequestSuccess(res.data)); //Here we are passing all the data from the API to...
            ...
            .catch(err => {                                 //If we are unable to send the data or make connection. 
                dispatch(deleteRequestFailure(err.message)); //We will get an error. The error is then passed to the error payload.
